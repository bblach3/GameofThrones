var btn = document.getElementById("btn")
var charPerPage = 50;
btn.addEventListener("click", function() {
    let fetchArr = [];
    let url = "";
    for(let page=0; page <45; page++){
        url = fetch("https://www.anapioficeandfire.com/api/characters?page=" + page + "&pageSize=" + charPerPage +"")
        fetchArr.push(url);
    }
    let promise = Promise.all(fetchArr);
    promise.then((resultsArr)=>{
        return Promise.all(resultsArr.map(char =>{
            return char.json()
        }))
    })
    .then((dataArr)=>{
        // console.log(dataArr[7][0].allegiances[0]);
        // dataArr[y][x].z
        // y is the number of characters on a page (50 => so in this case 0-49)
        // x is the number of pages (use .length to figure out how many pages were pulled)
        // z is how you access the property of the character
        let charList = [];
        let char = {};
        console.log(dataArr);
        // we need to figure out if x or y is going inside
        for(let y = 0; y < dataArr.length - 1; y++){
            for(let x = 0; x < dataArr[y].length; x++){
                // let { allegiances, name } = data;
                let allegiances = dataArr[y][x].allegiances[0];
                let name = dataArr[y][x].name;
                char = {
                    allegiances: allegiances,
                    name: name
                }
                // console.log(dataArr[y][x].allegiances);
                charList.push(char);
                // At end of each iteration, reset your char obj
                char = {};
            }
        }
        // charList => [{}, {}, {}] where {} = to each character's info.
        //dom manipulation right here
        renderHTML(charList)
    })
})
function renderHTML(data) {
    var htmlString = "";
    var row = document.querySelector('.row')
//"<p>" + data[i].name + ".</p>"
    for (i =0; i < data.length; i++) {
        htmlString +=`
    <div class="col-4">
        <div class="card">
            <div class="card-header">
            
            
              <a href="#" class="card-header">${data[i].name} <b></b>  </a>
            </div>
          </div>
    </div>
`
//${data[i].allegiances}

    }
    // charContainer.insertAdjacentHTML('beforeend', htmlString )
    row.innerHTML = htmlString
    // tags = charList.map((char)=>{
    //     return `<a href="${char.url}" class="card-header">${char.name}<b>houses</b> ${char.allegiances.length}</a>`
    // })
    let $div = $(".row")
        $div.click((e)=> {
            e.preventDefault();
            // console.log(e.target.href)
            // $.get(e.target.href)

            let fetchUrl = e.target.textContent.slice(8);
           
            if (fetchUrl !== undefined){
                $.get(fetchUrl)
                .done((detailedCharObj)=> {
    
                let $modalBody = $('.modal-body');
    
                let $modalTitle = $('#exampleModalLabel');
    
               // $modalBody.html("");
                $modalBody.html(detailedCharObj.name);
    
                $modalTitle.html('HOUSE')
    
                console.log(detailedCharObj);
                console.log(detailedCharObj.name);
               
               
    
                // if(detailedCharObj.allegiances > 0){
                //     detailedCharObj.allegiances.forEach((houseUrl)=>{
                //         $.get(houseUrl)
                //         .done((houseObj)=>{
                //             $modalBody.html(`<br>${$modalBody.html()}<br>${houseObj.name}`)
                //         })
                //     })
    
                // }
    
                $('#exampleModal').modal('show');
            })
            } //end of done statement
        })
        // const div = document.querySelector('.row');
        // div.addEventListener('click', event => {
        //     //console.log('huhuh')
        //     console.log(event.target.fetchArr)
        // });
    }
    // let apiCall1 = fetch("https://www.anapioficeandfire.com/api/characters?page=1&pageSize=50")
    // let apiCall2 = fetch("https://www.anapioficeandfire.com/api/characters?page=1&pageSize=50")
    // let promise = Promise.all([apiCall1, apiCall2])
    // promise.then((resultsArr)=>{
    //     let results = [];
    //     results[0] = resutlsArr[0].json();
    //     results[1] = resutlsArr[1].json();
    //     return Promise.all(results)
    // })
    // .then((dataArr)=>{
    //     console.log(dataArr);
    //     let charArr = []
    //     dataArr.forEach((char)=>{
    //         charArray = [...charArray, ...char]
    //     })
    //     console.log(charArray);
    //     //manipulate the dom
    // })
// 45 fetch calls concurrently with promise all
// Because javascript is asynchronous, there's no way we can interact with both of these datasets in memory
// Each character you add a button and attach a click event
// Then have it fetch to the houseURL when that click even happens
// then display fetched data